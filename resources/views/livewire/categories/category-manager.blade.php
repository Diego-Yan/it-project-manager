<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-5">
        ...
        <div class="space-y-4">
            ...
            {{-- 分类名称 --}}
            <div>
                <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5 block">分类名称</label>
                <input type="text" wire:model="name" placeholder="如：网络运维 / HR系统"
            ...
            {{-- 颜色 --}}
            <div>
                <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5 block">颜色标签</label>
                @php
                $colorPalette = [
                    'red'    => ['label' => '赤', 'hex' => '#ef4444'],
                    'orange' => ['label' => '橙', 'hex' => '#f97316'],
                    'yellow' => ['label' => '黄', 'hex' => '#eab308'],
                    'green'  => ['label' => '绿', 'hex' => '#22c55e'],
                    'cyan'   => ['label' => '青', 'hex' => '#06b6d4'],
                    'blue'   => ['label' => '蓝', 'hex' => '#3b82f6'],
                    'purple' => ['label' => '紫', 'hex' => '#a855f7'],
                ];
                @endphp
            ...
            {{-- 描述 --}}
            <div>
                <label class="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5 block">描述（可选）</label>
                <input type="text" wire:model="description" placeholder="简短描述此分类"
                    class="w-full px-4 py-2.5 text-sm bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl text-zinc-900 dark:text-white placeholder-zinc-400 focus:outline-none focus:border-sky-500">
            </div>
            ...
            <div class="flex gap-2 justify-end">
                ...
                <button wire:click="save"
                    class="px-5 py-2 text-sm font-medium text-white bg-sky-600 hover:bg-sky-500 rounded-xl transition-colors">保存</button>
            </div>
        </div>
    </div>
    @endif
    ...
    <div>
        <div class="flex items-center gap-2 mb-3">
            <h2 class="text-sm font-semibold text-zinc-700 dark:text-zinc-300">运维项目</h2>
            <span class="text-xs text-zinc-400">{{ $opsCategories->count() }} 个分类</span>
        </div>
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-5 hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors">
                ...
                @endif
                <span class="text-xs text-zinc-400">{{ $cat->projects_count }} 个项目</span>
            </div>
            @empty
            <div class="col-span-3 text-center py-8 text-zinc-400 text-sm">暂无运维项目分类</div>
            @endforelse
        </div>
    </div>
    ...
    <div>
        <div class="flex items-center gap-2 mb-3">
            <h2 class="text-sm font-semibold text-zinc-700 dark:text-zinc-300">开发项目</h2>
            <span class="text-xs text-zinc-400">{{ $devCategories->count() }} 个分类</span>
        </div>
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            @empty
            <div class="col-span-3 text-center py-8 text-zinc-400 text-sm">暂无开发项目分类</div>
            @endforelse
        </div>
    </div>
