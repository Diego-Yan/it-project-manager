<div class="space-y-5">
    ...
    <div class="space-y-3">
        <div class="flex items-center gap-3">
            <div class="relative flex-1">
                ...
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="{{ __('搜索项目名称...') }}"
            ...
            <a href="{{ route('projects.create') }}"
                ...
                <span class="hidden sm:inline">{{ __('新建项目') }}</span>
            </a>
            @endcan
        </div>
        ...
        <div class="space-y-2">
            @php
            $filterGroups = [
                __('进度') => [''=>'filterProgress', 'pending'=>__('未开始'),'in_progress'=>__('进行中'),'paused'=>__('已暂停'),'completed'=>__('已完成')],
                __('分类') => [''=>'filterCategory'],
                __('类型') => [''=>'filterType', 'new'=>__('新增'),'improved'=>__('改善'),'issue'=>__('异常')],
                __('紧急度') => [''=>'filterUrgency', 'not_urgent'=>__('不紧急'),'normal'=>__('一般'),'urgent'=>__('紧急')],
                __('重要性') => [''=>'filterImportance', 'normal'=>__('一般'),'important'=>__('重要'),'very_important'=>__('非常重要')],
            ];
            @endphp
            ...
            {{-- 地区 --}}
            <div class="flex items-center gap-1.5 flex-wrap">
                <span class="text-xs font-medium text-zinc-500 w-12 shrink-0">{{ __('地区') }}</span>
                @foreach($regions as $r)
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
        ...
        <div class="text-center py-16">
            ...
            @can('create projects')
            <a href="{{ route('projects.create') }}" class="mt-3 inline-block text-sky-600 text-sm hover:underline">{{ __('创建第一个项目') }}</a>
            @endcan
        </div>
        @else
        ...
        <div class="hidden md:grid grid-cols-12 gap-3 px-5 py-3 text-xs font-medium text-zinc-500 dark:text-zinc-400 border-b border-zinc-100 dark:border-zinc-800 uppercase tracking-wider">
            <div class="col-span-4">{{ __('项目名称') }}</div>
            <div class="col-span-1">{{ __('类型') }}</div>
            <div class="col-span-1">{{ __('分类') }}</div>
            <div class="col-span-1">{{ __('紧急度') }}</div>
            <div class="col-span-1">{{ __('重要性') }}</div>
            <div class="col-span-2">{{ __('进度') }}</div>
            <div class="col-span-1">{{ __('截止') }}</div>
            <div class="col-span-1 text-right">{{ __('操作') }}</div>
        </div>
        ...
        <div class="hidden md:grid grid-cols-12 gap-3 px-5 py-3.5 border-b border-zinc-50 dark:border-zinc-800/50 hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors items-center">
            <div class="col-span-4 min-w-0">
                ...
                <p class="text-xs text-zinc-400 mt-0.5">{{ __('创建于') }} {{ $project->created_at->format('Y/m/d') }}</p>
            </div>
            ...
            <div class="col-span-1">
                ...
                <span class="text-xs {{ $isOverdue ? 'text-red-600 dark:text-red-400 font-medium' : 'text-zinc-600 dark:text-zinc-400' }}">
                    ...
                    @if($isOverdue)<br><span class="text-xs">{{ __('逾期') }}</span>@endif
                </span>
                @else
            ...
            <div class="col-span-1 flex items-center justify-end gap-0.5">
                ...
                @if($hasApplied)
                <span class="text-xs text-amber-500 font-medium px-1.5" title="{{ __('等待审批') }}">{{ __('审批中') }}</span>
                @else
                ...
                <a href="{{ route('projects.show', $project) }}"
                    class="p-1.5 rounded-lg text-zinc-400 hover:text-sky-600 dark:hover:text-sky-400 hover:bg-sky-50 dark:hover:bg-sky-950/30 transition-colors" title="{{ __('查看详情') }}">
                ...
                <a href="{{ route('projects.edit', $project) }}"
                    class="p-1.5 rounded-lg text-zinc-400 hover:text-amber-600 dark:hover:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/30 transition-colors" title="{{ __('编辑') }}">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button wire:click="deleteProject({{ $project->id }})"
                    wire:confirm="{{ __('确定要删除「:title」吗？此操作不可恢复。', ['title' => $project->title]) }}"
                    class="p-1.5 rounded-lg text-zinc-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors" title="{{ __('删除') }}">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a href="{{ route('projects.show', $project) }}"
            ...
            <div class="flex items-center gap-2 text-xs flex-wrap">
                ...
                <span class="{{ $isOverdue ? 'text-red-500 font-medium' : 'text-zinc-500' }}">
                    {{ $project->end_date->format('m/d') }} {{ __('截止') }}
                </span>
                @else
                <span class="text-zinc-400">{{ __('无截止日') }}</span>
                @endif
            </div>
            <div class="flex items-center gap-2 text-xs mt-2">
                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium bg-{{ $uColor }}-100 dark:bg-{{ $uColor }}-950/40 text-{{ $uColor }}-700 dark:text-{{ $uColor }}-400">
                    {{ __('急:') }} {{ $project->urgencyLabel }}
                </span>
                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium bg-{{ $iColor }}-100 dark:bg-{{ $iColor }}-950/40 text-{{ $iColor }}-700 dark:text-{{ $iColor }}-400">
                    {{ __('重:') }} {{ $project->importanceLabel }}
                </span>
                <span class="text-zinc-300">|</span>
                <span class="text-zinc-500">{{ $project->completion_percent }}%</span>
                @if(!$isMember && $project->progress !== 'completed')
                @if($hasApplied)
                <span class="text-amber-500 font-medium ml-auto">{{ __('审批中') }}</span>
                @else
                <button wire:click="applyToProject({{ $project->id }})"
