<div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-5">
    ...
    <div class="mb-4 p-3 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-xl">
        <p class="text-sm font-medium text-red-700 dark:text-red-400 mb-1">等待依赖项目完成</p>
    ...
    <div class="mb-4 p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-200 dark:border-zinc-700">
        <div class="space-y-3">
            <div>
                <label class="block text-xs font-medium text-zinc-600 dark:text-zinc-400 mb-1">关系类型</label>
                <select wire:model="linkType"
                    ...
                    <option value="relates_to">↔ 关联引用</option>
                    <option value="blocks">→ 阻断依赖（A 完成后 B 才能开始）</option>
                    <option value="parent">← 设置父项目</option>
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" wire:model.live.debounce.200ms="searchLink" placeholder="搜索项目名称..."
            ...
            <div class="flex gap-2 justify-end">
                ...
                <button wire:click="addLink"
                    class="px-4 py-1.5 text-xs font-medium text-white bg-sky-600 hover:bg-sky-500 rounded-lg transition-colors">创建关联</button>
            </div>
        </div>
    </div>
    @endif
    ...
    <p class="text-xs text-zinc-400">暂无项目关联</p>
    @else
    <div class="space-y-2">
        ...
        <div class="flex items-center gap-3 p-2.5 rounded-xl bg-zinc-50 dark:bg-zinc-800/50">
            <span class="text-xs font-medium text-zinc-500 dark:text-zinc-400 shrink-0 w-20">
                {{ $link->link_type === 'parent' ? '← 父项目' : $link->linkTypeDirectionLabel }}
            </span>
            ...
            <button wire:click="removeLink({{ $link->id }})"
                wire:confirm="确定解除此关联？"
                class="text-xs text-zinc-400 hover:text-red-500 shrink-0">解除</button>
        </div>
        @endforeach
        ...
        <div class="flex items-center gap-3 p-2.5 rounded-xl bg-zinc-50 dark:bg-zinc-800/50">
            <span class="text-xs font-medium text-zinc-500 dark:text-zinc-400 shrink-0 w-20">
                {{ $link->link_type === 'parent' ? '→ 子项目' : ($link->link_type === 'blocks' ? '← 被阻断' : '↔ 被关联') }}
            </span>
