<div class="space-y-6">
    <div><h1 class="text-xl font-bold text-zinc-900 dark:text-white">企微 / 钉钉 集成</h1><p class="text-sm text-zinc-500 mt-1">自建应用接入：通讯录同步 + 消息接收 + 自动建工单</p></div>
    ...
    <div class="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-xl p-4 text-sm text-amber-800 dark:text-amber-300">
        <strong>不是群机器人：</strong>群机器人只能发消息。要接收消息和读通讯录，需要<strong>自建应用</strong>（企业微信）或<strong>企业内部应用</strong>（钉钉）。
    </div>

    {{-- ═══ 企业微信 ═══ --}}
    <div class="bg-white dark:bg-zinc-900 rounded-2xl border p-6">
        <h2 class="text-base font-semibold flex items-center gap-2"><span class="w-8 h-8 rounded-lg bg-green-100 dark:bg-green-950/40 flex items-center justify-center text-green-600 text-sm">微</span>企业微信 · 自建应用</h2>
        <p class="text-sm text-zinc-500 mt-2">在 <a href="https://work.weixin.qq.com/wework_admin/frame#apps" class="text-sky-500 underline" target="_blank">企微管理后台 → 应用管理 → 自建应用</a> 创建应用，获取 Corp ID 和 Secret。</p>
        ...
        <div class="mt-4 p-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl text-sm text-zinc-600 dark:text-zinc-400">
            <p class="font-medium mb-1">消息接收配置：</p>
            <p>自建应用 → 接收消息 → 设置 API 接收 → URL 填：<code class="bg-zinc-200 dark:bg-zinc-700 px-1.5 py-0.5 rounded text-xs">{{ $wechatUrl }}</code></p>
        </div>
    </div>

    {{-- ═══ 钉钉 ═══ --}}
    <div class="bg-white dark:bg-zinc-900 rounded-2xl border p-6">
        <h2 class="text-base font-semibold flex items-center gap-2"><span class="w-8 h-8 rounded-lg bg-blue-100 dark:bg-blue-950/40 flex items-center justify-center text-blue-600 text-sm">钉</span>钉钉 · 企业内部应用</h2>
        <p class="text-sm text-zinc-500 mt-2">在 <a href="https://open.dingtalk.com/" class="text-sky-500 underline" target="_blank">钉钉开放平台 → 应用开发 → 企业内部应用</a> 创建应用，获取 App Key 和 Secret。</p>
        ...
        <div class="mt-4 p-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl text-sm text-zinc-600 dark:text-zinc-400">
            <p class="font-medium mb-1">消息接收配置：</p>
            <p>应用 → 机器人 → 消息接收模式 → HTTP 模式 → URL 填：<code class="bg-zinc-200 dark:bg-zinc-700 px-1.5 py-0.5 rounded text-xs">{{ $dingtalkUrl }}</code></p>
        </div>
    </div>

    {{-- 机器人命令 --}}
    <div class="bg-white dark:bg-zinc-900 rounded-2xl border p-6">
        <h2 class="text-base font-semibold mb-3">消息命令</h2>
        <div class="space-y-2 text-sm">
            <div class="flex items-start gap-3"><span class="shrink-0 px-2 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800 text-xs font-mono">3楼打印机没墨了</span><span class="text-zinc-500">→ 自动创建工单</span></div>
            <div class="flex items-start gap-3"><span class="shrink-0 px-2 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800 text-xs font-mono">状态 5</span><span class="text-zinc-500">→ 查询 #5 工单进度</span></div>
            <div class="flex items-start gap-3"><span class="shrink-0 px-2 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800 text-xs font-mono">紧急 服务器宕机了</span><span class="text-zinc-500">→ 创建高优先级工单</span></div>
        </div>
    </div>
</div>
