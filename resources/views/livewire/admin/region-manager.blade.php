<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div><h1 class="text-xl font-bold text-zinc-900 dark:text-white">地区管理</h1><p class="text-sm text-zinc-500 mt-1">管理工单和项目归属地区</p></div>
        <button wire:click="$set('showForm', true)" class="px-4 py-2 bg-sky-600 hover:bg-sky-500 text-white text-sm font-medium rounded-xl">+ 新增地区</button>
    </div>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-2xl border p-5">
        <h3 class="text-sm font-semibold mb-4">{{ $editingId ? '编辑地区' : '新增地区' }}</h3>
        <div class="flex gap-3 items-end">
            <div class="flex-1"><label class="text-xs font-medium text-zinc-500">地区名称</label><input wire:model="formName" wire:keydown.enter="save" placeholder="输入地区名称" class="w-full mt-1 px-3 py-2 text-sm border rounded-xl dark:bg-zinc-800 dark:border-zinc-700"></div>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-2xl border overflow-hidden">
        @if($regions->isEmpty())
        <div class="text-center py-16 text-zinc-400 text-sm">暂无地区</div>
        @else
        <div class="divide-y divide-zinc-100 dark:divide-zinc-800">
            ...
            <div class="flex items-center gap-4 px-5 py-4 hover:bg-zinc-50 dark:hover:bg-zinc-800/30">
                ...
                <button wire:click="delete({{ $r->id }})" wire:confirm="确定删除「{{ $r->name }}」？" class="p-1.5 text-zinc-400 hover:text-red-500"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
            </div>
            @endforeach
        </div>
        @endif
