<div class="space-y-6" x-data="{}">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-semibold text-zinc-900 dark:text-white">AD 域认证配置</h1>
            <p class="text-sm text-zinc-500 mt-0.5">配置 Windows Active Directory 域认证，允许域账号直接登录本系统。</p>
        </div>
        <div class="flex items-center gap-3">
            <button wire:click="testConnection" wire:loading.attr="disabled"
                ...
                <span wire:loading.remove wire:target="testConnection">测试连接</span>
                <span wire:loading wire:target="testConnection">连接中...</span>
            </button>
            <button wire:click="save" wire:loading.attr="disabled"
                ...
                <span wire:loading.remove wire:target="save">保存配置</span>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
        <div class="flex items-center justify-between px-6 py-4 border-b border-zinc-100 dark:border-zinc-800">
            <div>
                <h2 class="text-sm font-semibold text-zinc-900 dark:text-white">启用 AD 域认证</h2>
                <p class="text-xs text-zinc-500 mt-0.5">开启后，用户可使用域账号登录；关闭则仅允许本地账号。</p>
            </div>
        ...
        <div class="px-6 py-3 bg-sky-50 dark:bg-sky-950/20 border-b border-sky-100 dark:border-sky-900/50 text-xs text-sky-700 dark:text-sky-400 flex items-center gap-2">
            ...
            AD 认证已启用。登录页面将显示「AD 域账号登录」选项，域用户输入 <strong>域用户名</strong>（不含域名前缀）和密码即可登录。
        </div>
        @endif
    </div>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
        <div class="px-6 py-4 border-b border-zinc-100 dark:border-zinc-800">
            <h2 class="text-sm font-semibold text-zinc-900 dark:text-white">服务器连接</h2>
            <p class="text-xs text-zinc-500 mt-0.5">AD 服务器地址和通信协议配置。</p>
        </div>
        <div class="px-6 py-5 space-y-4">

            <div class="grid grid-cols-3 gap-4">
                <div class="col-span-2">
                    <label class="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">AD 服务器 IP / 主机名 <span class="text-red-500">*</span></label>
                    <input type="text" wire:model="adServer" placeholder="如: 192.168.0.25 或 ad.company.com"
                ...
                <div>
                    <label class="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">端口 <span class="text-red-500">*</span></label>
                    ...
                    <p class="mt-1 text-xs text-zinc-400">标准: 389 / SSL: 636</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="flex items-center gap-3 px-4 py-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-200 dark:border-zinc-700">
                    ...
                    <div>
                        <label for="adUseTls" class="text-sm font-medium text-zinc-700 dark:text-zinc-300 cursor-pointer">启用 TLS（STARTTLS）</label>
                        <p class="text-xs text-zinc-400">在 389 端口使用加密通信</p>
                    </div>
                </div>
                <div class="flex items-center gap-3 px-4 py-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-200 dark:border-zinc-700">
                    <input type="checkbox" wire:model="adUseSsl" id="adUseSsl"
                        class="w-4 h-4 rounded border-zinc-300 text-sky-600 focus:ring-sky-500">
                    <div>
                        <label for="adUseSsl" class="text-sm font-medium text-zinc-700 dark:text-zinc-300 cursor-pointer">启用 SSL（LDAPS）</label>
                        <p class="text-xs text-zinc-400">使用 LDAPS（端口通常为 636）</p>
                    </div>
                </div>
            </div>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
        <div class="px-6 py-4 border-b border-zinc-100 dark:border-zinc-800">
            <h2 class="text-sm font-semibold text-zinc-900 dark:text-white">域信息</h2>
            <p class="text-xs text-zinc-500 mt-0.5">Windows 域名和 LDAP 搜索基路径。</p>
        </div>
        <div class="px-6 py-5 space-y-4">

            <div>
                <label class="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">域名（NetBIOS / FQDN）<span class="text-red-500">*</span></label>
                <input type="text" wire:model="adDomain" placeholder="如: company.com 或 COMPANY"
                ...
                <p class="mt-1 text-xs text-zinc-400">用于构造登录名：username@{{ $adDomain ?: 'domain.com' }}</p>
                @error('adDomain')<p class="mt-1 text-xs text-red-500">{{ $message }}</p>@enderror
            </div>

            <div>
                <label class="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">搜索基路径（Base DN）<span class="text-red-500">*</span></label>
                <input type="text" wire:model="adBaseDn" placeholder="如: DC=company,DC=com"
                ...
                <p class="mt-1 text-xs text-zinc-400">LDAP 查询用户的起始路径，通常与域名对应。</p>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
        <div class="px-6 py-4 border-b border-zinc-100 dark:border-zinc-800">
            <h2 class="text-sm font-semibold text-zinc-900 dark:text-white">查询账号</h2>
            <p class="text-xs text-zinc-500 mt-0.5">用于在 AD 中查询用户信息的账号（需要读取权限，建议使用专用服务账号）。</p>
        </div>
        <div class="px-6 py-5 space-y-4">

            <div>
                <label class="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">管理员用户名</label>
                <input type="text" wire:model="adAdminUsername" placeholder="如: svc-ldap 或 administrator"
                    class="w-full px-3 py-2.5 text-sm bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl text-zinc-900 dark:text-white placeholder-zinc-400 focus:outline-none focus:border-sky-500 transition-colors">
                <p class="mt-1 text-xs text-zinc-400">填写域用户名（不含域名），登录时系统自动添加 @域名 后缀。</p>
            </div>

            <div>
                <label class="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">管理员密码</label>
                <div class="relative" x-data="{ show: false }">
                    <input :type="show ? 'text' : 'password'" wire:model="adAdminPassword"
                        placeholder="{{ config('ad-auth.admin_password') ? '（已设置，留空则不修改）' : '请输入密码' }}"
                ...
                <p class="mt-1 text-xs text-zinc-400">留空则不更改已存储的密码。密码将加密保存在服务器配置中。</p>
            </div>
    ...
    <div class="grid grid-cols-2 gap-6">
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
            <div class="px-6 py-4 border-b border-zinc-100 dark:border-zinc-800">
                <h2 class="text-sm font-semibold text-zinc-900 dark:text-white">用户同步</h2>
                <p class="text-xs text-zinc-500 mt-0.5">域用户首次登录时的处理方式。</p>
            </div>
            <div class="px-6 py-5 space-y-4">

                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-zinc-700 dark:text-zinc-300">自动创建本地用户</p>
                        <p class="text-xs text-zinc-400 mt-0.5">域用户首次登录时自动在系统中建立账号</p>
                    </div>
                    <label class="relative inline-flex items-center cursor-pointer">
                ...
                <div>
                    <label class="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">新用户默认角色</label>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-zinc-700 dark:text-zinc-300">同步 AD 用户组</p>
                        <p class="text-xs text-zinc-400 mt-0.5">自动根据 AD 组映射系统角色</p>
                    </div>
                    <label class="relative inline-flex items-center cursor-pointer">
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
            <div class="px-6 py-4 border-b border-zinc-100 dark:border-zinc-800">
                <h2 class="text-sm font-semibold text-zinc-900 dark:text-white">安全策略</h2>
                <p class="text-xs text-zinc-500 mt-0.5">登录失败保护和回退认证设置。</p>
            </div>
            <div class="px-6 py-5 space-y-4">

                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-zinc-700 dark:text-zinc-300">AD 失败时回退本地认证</p>
                        <p class="text-xs text-zinc-400 mt-0.5">AD 不可用时允许使用本地账号登录</p>
                    </div>
                    <label class="relative inline-flex items-center cursor-pointer">
                ...
                <div>
                    <label class="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">连续失败锁定次数</label>
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm text-zinc-500">次失败后锁定账号</span>
                    </div>
                </div>

                <div>
                    <label class="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">锁定时长</label>
                    <div class="flex items-center gap-2">
                        <input type="number" wire:model="adLockMinutes" min="1" max="1440"
                            class="w-24 px-3 py-2.5 text-sm bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl text-zinc-900 dark:text-white focus:outline-none focus:border-sky-500 transition-colors">
                        <span class="text-sm text-zinc-500">分钟</span>
                    </div>
                </div>
    ...
    <div class="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/50 rounded-2xl px-6 py-4">
        ...
        <div class="text-xs text-amber-700 dark:text-amber-400 space-y-1.5">
            <p>• <strong>深圳 AD 主域控</strong>: 192.168.0.25 &nbsp;|&nbsp; <strong>备域控</strong>: 192.168.0.26</p>
            <p>• <strong>杭州 AD 主域控</strong>: 10.10.0.8 &nbsp;|&nbsp; <strong>备域控</strong>: 10.10.0.10</p>
            <p>• Base DN 示例: <code class="bg-amber-100 dark:bg-amber-900/50 px-1 rounded">DC=company,DC=com</code>（将 company 和 com 替换为实际域名各段）</p>
            <p>• 填写后点击 <strong>「测试连接」</strong> 验证服务器是否可达，再点 <strong>「保存配置」</strong> 生效。</p>
        </div>
    </div>
