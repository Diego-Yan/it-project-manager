<div class="space-y-5">
    <div class="flex items-center justify-between">
        <div><h1 class="text-xl font-bold text-zinc-900 dark:text-white">{{ __('知识库') }}</h1><p class="text-sm text-zinc-500 mt-1">{{ __('常见问题 · 操作手册 · 经验沉淀 · 文件归档') }}</p></div>
        <button wire:click="$set('showForm', true)" class="px-4 py-2 bg-sky-600 hover:bg-sky-500 text-white text-sm font-medium rounded-xl">{{ __('+ 写文章') }}</button>
    </div>
    ...
    <div class="flex gap-3 items-center flex-wrap">
        <input wire:model.live.debounce.300ms="search" placeholder="{{ __('搜索文章标题或内容...') }}" class="flex-1 min-w-48 px-4 h-10 text-sm border rounded-xl dark:bg-zinc-800 dark:border-zinc-700">
        <select wire:model.live="filterTag" class="px-3 h-10 text-sm border rounded-xl dark:bg-zinc-800 dark:border-zinc-700">
            <option value="">{{ __('全部标签') }}</option>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-2xl border p-5">
        <h3 class="text-sm font-semibold mb-4">{{ $editingId ? __('编辑文章') : __('撰写文章') }}</h3>
        <input wire:model="formTitle" placeholder="{{ __('文章标题*') }}" class="w-full px-3 h-10 text-sm border rounded-xl dark:bg-zinc-800 dark:border-zinc-700 mb-3">
        <div class="grid sm:grid-cols-2 gap-3 mb-3">
            <select wire:model="formCategory" class="px-3 h-10 text-sm border rounded-xl dark:bg-zinc-800 dark:border-zinc-700">
                <option value="general">{{ __('通用') }}</option><option value="hardware">{{ __('硬件') }}</option><option value="software">{{ __('软件') }}</option><option value="network">{{ __('网络') }}</option><option value="account">{{ __('账号') }}</option><option value="printer">{{ __('打印') }}</option>
            </select>
            <div>
                <label class="block text-xs font-medium text-zinc-500 mb-1">{{ __('附件 (PDF/Word/PPT/图片)') }}</label>
                ...
                <div wire:loading wire:target="uploadFile" class="text-xs text-sky-500 mt-1">{{ __('上传中...') }}</div>
                @error('uploadFile')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror
            </div>
        </div>
        <textarea wire:model="formContent" rows="6" placeholder="{{ __('文章内容* (支持 Markdown)') }}" class="w-full px-3 min-h-[5rem] text-sm border rounded-xl dark:bg-zinc-800 dark:border-zinc-700 mb-3"></textarea>
        {{-- 标签选择 --}}
        <div class="mb-3">
            <label class="block text-xs font-medium text-zinc-500 mb-2">{{ __('标签') }}</label>
            <div class="flex gap-2 flex-wrap">
        ...
        <div class="flex gap-2 justify-end"><button wire:click="resetForm" class="px-4 py-2 text-sm text-zinc-500">{{ __('取消') }}</button><button wire:click="save" class="px-4 py-2 text-sm font-medium bg-sky-600 text-white rounded-xl">{{ __('发布') }}</button></div>
    </div>
    @endif
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-2xl border p-6">
        <button wire:click="$set('viewId', null)" class="text-xs text-zinc-400 hover:text-sky-500 mb-3">{{ __('← 返回列表') }}</button>
        ...
        <div class="flex items-center gap-2 flex-wrap text-xs text-zinc-500 mb-4">
            ...
            <span>· {{ $viewArticle->view_count }} {{ __('次阅读') }}</span>
        ...
        <div class="mb-4 space-y-2">
            <p class="text-sm font-medium text-zinc-700 dark:text-zinc-300">{{ __('附件') }} ({{ $viewArticle->attachments->count() }})</p>
            ...
            <div class="flex items-center gap-3 p-3 rounded-xl bg-zinc-50 dark:bg-zinc-800/50">
                ...
                <a href="{{ asset('storage/'.$att->file_path) }}" target="_blank" class="px-3 py-1.5 text-xs bg-sky-600 text-white rounded-lg hover:bg-sky-500">{{ __('下载/预览') }}</a>
            </div>
            @endforeach
        </div>
        @endif
    ...
    <div class="grid md:grid-cols-2 gap-4">
        ...
        <button wire:click="view({{ $a->id }})" class="text-left bg-white dark:bg-zinc-900 rounded-2xl border p-5 hover:border-sky-300 dark:hover:border-sky-700 transition-all">
            ...
            <div class="flex items-center gap-2 text-xs text-zinc-400 flex-wrap">
                ...
                <span class="ml-auto">{{ $a->author->name }} · {{ $a->view_count }}{{ __('次') }}</span>
            </div>
        </button>
        @endforeach
    </div>
