<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div><h1 class="text-xl font-bold text-zinc-900 dark:text-white">Zabbix 集成</h1><p class="text-sm text-zinc-500 mt-1">对接 Zabbix 监控，告警自动生成故障工单</p></div>
        <button wire:click="$set('showForm', true)" class="px-4 py-2 bg-sky-600 hover:bg-sky-500 text-white text-sm font-medium rounded-xl">+ 添加 Zabbix</button>
    </div>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-2xl border overflow-hidden">
        @if($configs->isEmpty())
        <div class="text-center py-16 text-zinc-400 text-sm">暂无 Zabbix 连接</div>
        @else
            ...
            <div class="px-5 py-4 hover:bg-zinc-50 dark:hover:bg-zinc-800/30 flex items-center gap-4">
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 flex-wrap mb-1">
                        ...
                        <span class="text-xs {{ $c->is_active ? 'text-green-600' : 'text-zinc-400' }}">{{ $c->is_active ? '● 启用' : '○ 停用' }}</span>
                        <span class="text-xs text-zinc-500">级别 ≥ {{ App\Services\ZabbixService::severityLabel($c->min_severity) }}</span>
                        <span class="text-xs text-zinc-400">每 {{ $c->poll_interval }}分钟</span>
                    </div>
                    <p class="text-xs text-zinc-400 truncate">{{ $c->url }}</p>
                    @if($c->last_poll_at)<p class="text-xs text-zinc-500">上次轮询: {{ $c->last_poll_at->format('m/d H:i') }}</p>@endif
                </div>
                <div class="flex gap-1 shrink-0">
                    <button wire:click="test({{ $c->id }})" class="px-3 py-1.5 text-xs text-sky-600 border border-sky-300 rounded-lg hover:bg-sky-50">测试</button>
                    ...
                    <button wire:click="delete({{ $c->id }})" wire:confirm="确定删除？" class="p-1.5 text-zinc-400 hover:text-red-500"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
                </div>
            </div>
            @endforeach
        </div>
    ...
    <div class="p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl text-sm text-zinc-600 dark:text-zinc-400">
        <p class="font-medium mb-2">Cron 定时任务:</p>
        ...
        <p class="text-xs mt-2 text-zinc-500">命令会检查所有启用的 Zabbix 连接，将新告警生成工单（已有工单的去重跳过）</p>
    </div>
</div>
