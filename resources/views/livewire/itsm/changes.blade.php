<div class="space-y-5">
    <div class="flex items-center justify-between">
        <div><h1 class="text-xl font-bold text-zinc-900 dark:text-white">变更管理</h1><p class="text-sm text-zinc-500 mt-1">ITIL 变更请求：发布、配置、回滚、紧急修复</p></div>
        <button wire:click="$set('showForm', true)" class="px-4 py-2 bg-sky-600 hover:bg-sky-500 text-white text-sm font-medium rounded-xl">+ 新建变更</button>
    </div>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-5">
        ...
        <textarea wire:model="formDescription" rows="2" placeholder="变更描述" class="w-full mt-3 px-3 min-h-[5rem] text-sm border rounded-xl dark:bg-zinc-800 dark:border-zinc-700"></textarea>
        @error('formTitle')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror
        <textarea wire:model="formRollbackPlan" rows="2" placeholder="回滚方案" class="w-full mt-3 px-3 min-h-[5rem] text-sm border rounded-xl dark:bg-zinc-800 dark:border-zinc-700"></textarea>
        @error('formTitle')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
        @if($changes->isEmpty())
        <div class="text-center py-16 text-zinc-400 text-sm">暂无变更记录</div>
        @else
        <div class="divide-y divide-zinc-100 dark:divide-zinc-800">
            ...
            <div class="px-5 py-4 hover:bg-zinc-50 dark:hover:bg-zinc-800/30">
                <div class="flex items-center gap-3 mb-1.5 flex-wrap">
                    ...
                    <span class="inline-flex px-2 py-0.5 rounded text-xs font-medium bg-{{ $cr->riskColor }}-100 dark:bg-{{ $cr->riskColor }}-950/40 text-{{ $cr->riskColor }}-700 dark:text-{{ $cr->riskColor }}-400">{{ $cr->riskLabel }}风险</span>
                ...
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    {{-- Action buttons --}}
                    @if($cr->status === 'draft')
                    <button wire:click="submitForApproval({{ $cr->id }})" class="text-xs text-amber-600 hover:underline ml-2">提交审批</button>
                    @endif
                    @if($cr->status === 'pending_approval')
                    <button wire:click="approve({{ $cr->id }})" class="text-xs text-green-600 hover:underline ml-2">批准</button>
                    <button wire:click="reject({{ $cr->id }})" class="text-xs text-red-500 hover:underline">拒绝</button>
                    @endif
                    @if($cr->status === 'approved')
                    <button wire:click="startImplement({{ $cr->id }})" class="text-xs text-sky-600 hover:underline ml-2">开始执行</button>
                    @endif
                    @if($cr->status === 'in_progress')
                    <button wire:click="complete({{ $cr->id }})" class="text-xs text-green-600 hover:underline ml-2">完成</button>
                    <button wire:click="rollback({{ $cr->id }})" class="text-xs text-red-500 hover:underline">回滚</button>
                    @endif
                    @if(in_array($cr->status, ['draft','rejected']))
                    <button wire:click="edit({{ $cr->id }})" class="text-xs text-zinc-400 hover:text-sky-500 ml-2">编辑</button>
                    <button wire:click="delete({{ $cr->id }})" wire:confirm="删除？" class="text-xs text-zinc-400 hover:text-red-500">删除</button>
                    @endif
                </div>
            </div>
            @endforeach
